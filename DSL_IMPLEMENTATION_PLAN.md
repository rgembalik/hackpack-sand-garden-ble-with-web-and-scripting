...
| Item | Limit |
|------|-------|
| Max script length (chars) | 768 (fits small BLE upload, adjustable) |
| Max tokens | 240 |
| Max ops per assignment | 128 (tunable) |
| Stack depth | 24 (verify still sufficient with clamp(sign()) nesting; adjust if temporal scripts push limits) |
| Number of assignments | Up to 20 (outputs + locals combined) |
| Max locals | 12 |
